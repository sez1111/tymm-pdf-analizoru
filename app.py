...
APP_VERSION = "v7 - TYMM YAZILI GELİŞTİRME ASİSTANI"
...
st.set_page_config(page_title="TYMM v7 Yazılı Geliştirme Asistanı", layout="wide")
st.title("TYMM Program Tabanlı Yazılı Geliştirme Asistanı")
st.caption(APP_VERSION)
st.info("Hedef kazanım seçimi opsiyoneldir. Seçmezseniz sistem seçilen sınıfın tüm çıktıları içinden otomatik analiz yapar.")
...
if not program_pdf:
    ...
    if run:
        if not exam_pdf:
            ...
            with st.spinner("Yazılı okunuyor; öğrenme çıktısı ve süreç bileşeni kapsaması analiz ediliyor..."):
            ...
            with tab5:
                ...
                for a in analyses:
                    report.append(f"Soru {a['no']}: {a['primary'].code} | Süreç: {a['process_count']} | Eşleşen: {', '.join(a['matched_processes']) or '-'} | Desteklenebilecek: {', '.join(a['missing_processes']) or '-'} | Öneri: {a['suggestion']}")
                report_text = "\n".join(report)
