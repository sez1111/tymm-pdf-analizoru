...
APP_VERSION = "v9 - ÖĞRETMEN PLANLI SORU BAZLI ANALİZ"
...
def split_questions(text: str) -> List[Question]:
    ...
    for q in sorted(questions, key=lambda x: x.no):
        key = (q.no, re.sub(r"\s+", " ", q.text[:220]))
        if key not in seen:
            unique.append(q)
            seen.add(key)
    return unique
...
COMMON_PROCESS_OPTIONS = [
    "açıklama", "karşılaştırma", "yorumlama", "çıkarım yapma",
    "gerekçelendirme", "sorgulama", "sınıflandırma",
    "değerlendirme", "neden-sonuç kurma", "analiz",
    "ilişkilendirme", "modelleme", "deney tasarlama"
]
...
def analyze_question_against_plan(q: Question, plan: Dict):
    ...
    return {
        ...
        "process_count": f"{len(matched)}/{total}" if total else "Belirsiz",
        "process_ratio": ratio,
        "evidence": evidence,
        "status": quality_level(ratio),
        "suggestion": improvement_suggestion(matched, missing, evidence),
    }
...
st.set_page_config(page_title="TYMM v9 Öğretmen Planlı Analiz", layout="wide")
st.title("TYMM Yazılı Geliştirme Asistanı")
st.caption(APP_VERSION)

st.info("Bu sürümde program PDF yoktur. Öğretmen ana soru sayısını, her soru için öğrenme çıktısını ve hedef süreç bileşenlerini girer. Sistem yazılı PDF'ini doğrudan bu plana göre değerlendirir.")
...
st.caption("Her ana soru için hedef öğrenme çıktısını ve süreç bileşenlerini girin. Alt maddeler varsa yine aynı ana soru altında değerlendirilir.")
    ...
    for i in range(1, int(question_count) + 1):
        with st.expander(f"Soru {i} hedefleri", expanded=(i <= 3)):
            ...
            selected_common = st.multiselect(
                f"Soru {i} yaygın süreç bileşenleri",
                COMMON_PROCESS_OPTIONS,
                key=f"common_proc_{i}"
            )
            raw_proc = st.text_area(
                f"Soru {i} ek süreç bileşenleri",
                key=f"proc_{i}",
                placeholder="İsterseniz virgülle ek süreç yazın: örn. çıkarım yapma, gerekçelendirme",
                height=70
            )
            processes = selected_common + [p for p in split_processes(raw_proc) if p not in selected_common]
            plan.append({
                "question_no": i,
...
if run:
    if not exam_pdf:
        ...
        with st.spinner("Yazılı PDF okunuyor ve ana sorular plana göre değerlendiriliyor..."):
            ...
            for p in plan:
                ...
                if q_no in q_map:
                    ...
                    analyses.append({
                        ...
                        "process_count": f"0/{len(p['processes'])}" if p["processes"] else "Belirsiz",
                        "process_ratio": 0,
                        "evidence": [],
